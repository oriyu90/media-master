...
- **Post-release on-device fixes (same v1.3.0, binary re-signed):** on-device testing (Android 14 emulator — the first real-device pass this release ever received before publishing) found and fixed four bugs that static checks (unit tests, lint, R8 build) could not catch:
  - **App crashed on every launch in the release build.** R8 was missing keep rules for WorkManager's internal Room database, so its generated `_Impl` class couldn't be found by reflection at startup — before any UI ever showed. This affected every signed release since R8 was enabled (v0.3.0+); it was never caught because no prior release had been installed on an actual device. Fixed with explicit ProGuard keep rules for Room/WorkManager classes.
  - **Library/Documents/Audio failed with "Invalid token LIMIT".** The MediaStore paging added in v1.1.0 appended `LIMIT`/`OFFSET` directly into the SQL sort-order string, which some MediaProvider versions (confirmed on API 34) reject outright. Fixed to use the proper `ContentResolver` query-args `Bundle` on API 26+, with a plain unpaged query as the API 24/25 fallback.
  - **Opening a `.tex` file crashed with a regex `PatternSyntaxException`.** A literal `}` needs escaping on Android's ICU regex engine even though desktop-JVM regex (where the unit tests run) accepts it unescaped — a gap unit tests structurally cannot cover. Fixed the escaping; this is now called out in-repo as a class of bug that requires on-device testing to catch.
  - **Inline LaTeX math could render wrapped one-character-per-line.** The self-sizing WebView started at a near-zero width before its first measurement, and `horizontalScroll` on the parent gave it an unbounded width constraint that defeated `fillMaxWidth()`. Fixed by removing the unneeded `horizontalScroll` wrapper around math views.
  - The Documents list also showed "PDF document" as the type label for every non-PDF file; it now shows the actual file-type extension (e.g. "CSV", "DOCX").
- Version `1.3.0` (`versionCode 7`), signed with the same upload key as v0.1.0–v1.2.0 (APK Signature Scheme v2+v3 verified). **Final SHA-256 (post-fix): `500ec32227858828e81370fc6b5d90f39495fc8b081a398a860610953ad06058`.**
